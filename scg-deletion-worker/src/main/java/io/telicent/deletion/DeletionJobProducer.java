...
public class DeletionJobProducer implements AutoCloseable {
    ...
    public Optional<RecordMetadata> sendDeletePatch(ConsumerRecord<Bytes, Bytes> record) throws ExecutionException, InterruptedException {
        ...
        if ("application/rdf-patch".equals(contentType)) {
            ...
            // if content type is unrecognized the record is skipped, and the job continues and completes as normal
            // same if an event is skipped because its payload is empty or can't be parsed
            if (lang == null) {
